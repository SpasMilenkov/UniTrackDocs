...
The backend API uses JSON configuration files that control various aspects of the application. Below is the actual `appsettings.Development.json` file used in the development environment:
...
```json
{
  ...
  "ConnectionStrings": {
    "Database": "Host=localhost; Port=5434; Database=UniTrackRemaster; Username=postgres; Password=admin"
  },
  "Jwt": {
    "Key": "",
...
1. **Logging**: Controls log verbosity levels for different parts of the application

   - `Default`: Overall logging level (Information, Warning, Error, etc.)
   - `Microsoft.AspNetCore`: Specific logging level for ASP.NET Core framework components

2. **Kestrel**: Configuration for the built-in web server

   - `Url`: The URL pattern the server will listen on

3. **ConnectionStrings**: Database connection information

   - `Database`: PostgreSQL connection string with host, port, database name, and credentials

4. **Jwt**: JSON Web Token authentication settings

   - `Key`: Secret key used for signing tokens (should be provided in production)
   - `Issuer`: The issuing authority for the JWT tokens
   - `Audience`: The intended audience for the tokens

5. **Gmail**: Settings for email notifications via Gmail

   - `EmailAddress`: The sender email address
   - `OAuth2`: OAuth 2.0 credentials for Gmail authentication

6. **TemplateSettings**: Email template configuration

   - `Source`: Source of email templates ("Embedded" means templates are embedded in the application)
   - `LocalPath`: Path to templates if using local files

7. **EmailProcessing**: Email handling settings

   - `TargetEmailAddress`: Destination email for certain notifications

8. **FirebaseCredentials**: Firebase configuration for cloud storage and notifications

   - `CredentialsPath`: Path to the Firebase service account JSON file
   - `BucketPath`: Storage bucket path

9. **FrontendUrl**: URL of the frontend application (for redirects and CORS)

10. **AllowedHosts**: Allowed host headers (asterisk means all hosts are allowed)

### Configuration in Different Environments

For production deployments, you should:

1. Use environment variables or Docker secrets for sensitive information like:
...
2. Create an `appsettings.Production.json` file with appropriate production settings:
...
```json
{
  ...
  "ConnectionStrings": {
    "Database": "Host=unitrack-db; Port=5432; Database=UniTrackRemaster; Username=postgres; Password=admin"
  },
...
You can override settings in several ways (in order of precedence):

1. **Command-line arguments**:

   ```bash
   dotnet run --Jwt:Key="your-secret-key"
   ```

2. **Environment variables**:

   ```bash
   export ConnectionStrings__Database="Host=db;..."
   ```
...
When running in Docker, you can set configuration values via environment variables in your docker-compose file:

```yaml
environment:
  - Logging__LogLevel__Default=Warning
  - ConnectionStrings__Database=Host=unitrack-db;Port=5432;Database=UniTrackRemaster;Username=postgres;Password=admin
